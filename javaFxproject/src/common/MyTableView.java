...
public class MyTableView<T> extends TableView<T> {
	...
	/**
	 * 
	 * @param table - fxml을 이용하여 table view를 만든경우 컬럼은 새로이
	 * 만들지 않고 만들어진 디자인된 컬럼에 CellValueFactory만 지정
	 * @param clazz - 사용할 vo 클래스 지정(확장자포함)
	 */
	public static void setTableView(TableView table,Class voclass) {
		...
		for (Field f : fields) {
			// CellFactory지정
			((TableColumn) table.getColumns().get(cnt)).
